...
Resource pools SHOULD use channels with a fixed capacity for bounded allocation. Always set a maximum size:
...
Graceful shutdown MUST use `signal.NotifyContext` for clean termination. All resources (connections, files, channels) MUST be drained before process exit:
